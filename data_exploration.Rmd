...
## Bicycle Crash Data in North Carolina
  ...
  *   Subgroup analyses could be done on types of crashes, county, city, region, etc.
...
bike_crashes_clean <- 
  ...
  mutate(
    ...
    CrashType = if_else(CrashType %in% c('Unknown Approach Paths', 'Unknown Location'), NA_character_, CrashType),
    ...
    NumLanes = factor(NumLanes, 
      levels = c('1 lane', '2 lanes', '3 lanes', '4 lanes', '5 lanes', '6 lanes', '7 lanes', '8 lanes', '9+ lanes')
    ),
    ...
    SpeedLimit = str_replace(SpeedLimit, '\\s{2}', ' '), # extra spaces between meaure and units have been removed
  ...
  mutate(
    ...
    BikeInjuryDichot = case_when(
      BikeInjuryCat %in% c('A' ,'K') ~ 1, 
      BikeInjuryCat %in% c('B', 'C', 'O') ~ 0,
      BikeInjuryCat %in% c('U') ~ NA_real_
    ),
    BikeInjuryDichotDisc = if_else(
      BikeInjuryDichot == 1, 'serious injury', 'nonserious injury'
    ),
    BikeInjuryDichotFct = factor(BikeInjuryDichotDisc, levels = c('serious injury', 'nonserious injury'))
  ) %>%
  filter(
    !is.na(BikeInjuryDichot)
  ) %>%
  mutate(
    BikeInjuryCat = fct_drop(BikeInjuryCat, 'U'),
    BikeInjuryDisc = fct_drop(BikeInjuryDisc, 'Unknown Injury')
  ) %>%
...
# find and remove any variables with less than 2 factors
bike_crashes_clean %>% 
  select(where(~is.factor(.x) & length(levels(.x)) < 2))
bike_crashes_clean <- bike_crashes_clean %>% select(-NumBicsUin) 
...
<!-- ```{r} -->
<!-- # Check which variables are zero or near-zero variance -->
<!-- nearZeroVar(bike_crashes_clean, saveMetrics=TRUE) %>% -->
<!--   filter(zeroVar | nzv) %>% -->
<!--   kable(caption = 'Zero / Non-zero Variance Variables') -->
<!-- # these will be left in so that PCA can attempt to use this information -->

<!-- # The near zero variables will be removed since cross-validation may create  -->
<!-- # underrepresented folds -->
<!-- bike_crashes_clean <- bike_crashes_clean %>%  -->
<!--   select( -->
<!--     -matches('Alc'), -->
<!--     -matches('NumBics'), -->
<!--     -AmbulanceR, # if ambulance used, too late, injury is severe, remove variable -->
<!--     -NumUnits, -->
<!--     -RdDefects, -->
<!--     -Workzone -->
<!--   ) -->

<!-- bike_crashes_clean %>% glimpse() -->
<!-- ``` -->

### Create Dummy Variables

Use these dummy variables (numerical) to investigate correlated variables

```{r}
bike_crash_dummies_mdl <- dummyVars(
  # keep BikeInjuryDichot in the matrix since already numeric
  ~ ., 
...
bike_crash_dummies_cor <- cor(
  bike_crash_dummies %>%
    select(-BikeInjuryDichot), # don't check the response variable
  use = 'complete.obs'
)
...
correlated_vars <- findCorrelation(
  ...
  cutoff = .75, # more conservative than default (.90)
  names = TRUE
)
...
bike_crash_dummies <- bike_crash_dummies %>%
  select(
    ...
    -matches('Disc'), # directly related to BikeInjuryDichot (response)
    -matches('Cat'), # directly related to BikeInjuryDichot (response)
    -(NumBicsAin1:NumBicsNoi2) # 
  )
