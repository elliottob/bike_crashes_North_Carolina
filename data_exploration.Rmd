...
# since approximately 13-14% are missing at most complete.obs should be fine
bike_crash_dummies %>%
  map(~ round(sum(is.na(.x)) / nrow(bike_crash_dummies) * 100, 1)) %>%
  as_tibble() %>%
  select(where(~ .x > 10))

# after filtering for complete cases, how many variables have zero variance?
bike_crash_dummies %>%
  filter(complete.cases(.)) %>%
  nearZeroVar(., saveMetrics=TRUE) %>%
  filter(zeroVar)

# compute correlation after removing zero variance variables
bike_crash_dummies_cor <- cor(
  bike_crash_dummies %>%
    select(
      -BikeInjuryDichot, # don't check the response variable
      -matches('City'), 
      -matches('(ATV)'), 
      -matches('Pedalcycle'), 
      -matches('Pedestrian'),
...
bike_crash_dummies <- bike_crash_dummies %>%
  select(
    -correlated_vars,
    -matches('Disc'), # related to BikeInjuryDichot
    -matches('City'), 
    -matches('(ATV)'), 
    -matches('Pedalcycle'), 
    -matches('Pedestrian'),
