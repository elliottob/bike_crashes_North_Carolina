...
mdl_performance <- function(mdl) {
  
  mdl_pred <- predict(mdl, newdata = test_dat)
  
  print(length(mdl_pred))
  print(length(test_dat[complete.cases(test_dat[,names(test_dat) %in% predictors(mdl)]),]$BikeInjuryDichot))
  
  conf_mat <- confusionMatrix(
    factor(mdl_pred, levels = c('Serious', 'Non.Serious')),
    test_dat[complete.cases(test_dat[,names(test_dat) %in% predictors(mdl)]),]$BikeInjuryDichot
  )
  ...
  perf <- 
    tibble(
      ...
      obs = list(test_dat[complete.cases(test_dat[,names(test_dat) %in% predictors(mdl)]),]$BikeInjuryDichot)
    ) %>%
    mutate(
      pred = map2(
        p, 
...
mdl_performance(binom_mdl)
predictors(binom_mdl)

# Logistic Classification with PCA 
mdl_performance(binom_PCA_mdl)
```
...
```{r}
# Naive Bayes
mdl_performance(nb_mdl)

# Random Forest 
mdl_performance(rf_mdl)
```

# Ensemble Predictions

```{r}
mdl_performance(ensemble_mdl)
```
