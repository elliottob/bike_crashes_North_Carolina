...
binomial_control <- trainControl(
  ...
  method = 'repeatedcv',
  repeats = 3,
  index = createFolds(bike_crashes_preProcessed$BikeInjurySerious, 10),
  seeds = seeds
)
...
log_PCA_perf <- mdl_test_performance(log_PCA_mdl, prob_cutoff = .452) %>%
...
```{r knn_pred}
# K-nearest Neighbors
mdl_train_performance(knn_mdl, conf_matrix = TRUE)
knn_mdl_perf <- mdl_test_performance(knn_mdl, prob_cutoff = .395) %>%
...
```{r rf_pred}
# Random Forest
mdl_train_performance(rf_mdl, conf_matrix = TRUE)
rf_mdl_perf <- mdl_test_performance(rf_mdl, prob_cutoff = .269) %>% 
...
However, the logistic model performs the best as far as balancing
sensitivity, accuracy and specificity. As stated before, PCA did not
make the logistic model perform better than without it and will not be
included in the ensemble. The naive Bayes model computes the fastest
with the least accuracy due to having a low specificity. The random
forest is the slowest to compute, and has second highest accuracy
(excluding PCA). K-nearest neighbors was the worst performer in accuracy
due to the lowest specificity and therefore, will not move on to the
ensemble model.
...
ensemble_control <- trainControl(
  ...
  repeats = 5,
  index = createFolds(bike_crashes_preProcessed$BikeInjurySerious, 10),
  # only need the first element of each seeds list 
  # since all hyper-params chosen
  seeds = seeds %>% map(1)
)
