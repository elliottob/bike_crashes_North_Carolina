...
preproc_mdl <- preProcess(
  ...
  method = c(
    ...
    'center',
    'scale',
    'medianImpute' # median impute for age
  )
)
...
and is not used in the ensemble reason for these reasons. Neural network also 
does not perform well since accuracy is low and speed is slow.
...
mdl_performance <- function(mdl, conf_matrix = FALSE, ensemble = FALSE, mdl_lst = NULL) {
  ...
  mdl_test_dat <- test_dat[complete.cases(test_dat[, names(test_dat) %in% mdl_predictors]),]
  
  # predictions based on training and test dataset
  mdl_train_pred <- predict(mdl, newdata = mdl_train_dat)
  mdl_test_pred <- predict(mdl, newdata = test_dat)
  ...
  if(conf_matrix) {
    test_conf_mat <- confusionMatrix(
      factor(mdl_test_pred, levels = c('Serious', 'Non.Serious')),
      factor(mdl_test_dat$BikeInjurySerious, levels = c('Serious', 'Non.Serious'))
    )
  }
  ...
  return(
    list(
      ...
      train_performance_tbl = perf,
      train_conf_matrix = train_conf_mat,
      test_conf_matrix = test_conf_mat
    )
  )
}
```
...
# Neural Network
mdl_performance(nn_mdl, conf_matrix = TRUE)
...
```{r model_runtimes}
tibble(
 binom_logistic = binom_time,
 binom_PCA = binom_PCA_time,
 naive_bayes = nb_time,
 random_forest = rf_time,
 neural_network = nn_time,
 ensemble = ensemble_time
) %>% kable(caption = 'Model Run-time')
```
