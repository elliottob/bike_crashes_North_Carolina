...
Assuming I work for the state of North Carolina as a data scientist, I'm working on a project that will investigate bicycle safety in the state of North Carolina in an effort to understand what factors to consider in decreasing the number serious bicycle injuries as caused by bicycle-vehicle collisions. Based on a 2018 progress report by the League of American Bicyclists (<https://bikeleague.org/sites/default/files/BFS_Progress_Report_2018_North_Carolina.pdf>), North Carolina is ranked as 43rd of 50 in respect to bicycle ridership with only .2% commuting via bicycle. According to the 2015-2019 report by North Carolina Department of Transportation (NCDOT, <https://www.pedbikeinfo.org/pbcat_nc/pdf/summary_bike_facts15-19.pdf>), 8.7% of bicycle-motor vehicle crashes resulted in fatal or suspected serious injuries in 2019. This statistic may not seem substantial, but as the state population and ridership increases, 8.7% can mean 100's of people seriously injured or killed in bicycle-motor vehicle crashes each year.

Bicycle commuting reduces the green house gas emissions by relying on human powered mechanics instead of fossil fuels or batteries. The cost, amount of raw materials and toxic processes is far less for a bicycle than for a motor vehicle, thus reducing environmental impact and resource management. The health advantages for a bicycle commuter compared to a motor-vehicle commuter are well known and can't be ignored. The advantage to bicycle commuting is clear for those that live close enough to work to bike. However, those that live farther out may require a supplementary transportation service to get them within biking range of work.

If North Carolina would like to increase the bicyle ridership, then safety of the public roads should be a main concern. In order to sustain safe bicycling conditions it would be necessary to know what the key environmental factors are in serious/fatal bike crashes.

In this report, we will explore machine learning models with the caret package in R and attempt to find the best model or ensemble model that can predict a bike injury being "serious" vs. "non-serious". The resulting model will be used to find the road, bicyclist, and driver features from the NCDOT bicycle crash database (2007-2019) that most affect the seriousness of a bicycle crash injury.
...
```{r describe_data, message=FALSE, warning=FALSE, echo=FALSE}
bike_crashes %>% 
  map_df(class) %>% 
  pivot_longer(
    -c(OBJECTID_1), 
...
The bike crashes dataset is sourced from the Department of Transportation in the state North Carolina from the years April 2007 - September 2019 ([North Carolina Bicycle and Pedestrian Crash Data Tool (pedbikeinfo.org)](https://www.pedbikeinfo.org/pbcat_nc/)). Raw data consists of 62 columns which are a mixture of numerical and character data types. The crash severity variable, CrashSevr, will be used as a response variable in a binary classification model, "Serious" vs. "Non-serious". Currently the variable is categorical with multiple levels of severity:
...
Five models will be tested for performance and used in an ensemble model with the hope that the ensemble model will be better at classification. The reason being that it's possible the ensemble model will balance the importance of key features that predict injury seriousness. The five models to be tested are as follows:

-   Logistic Regression (with ElasticNet regularization)

-   Logistic Regression (with ElasticNet regularization and principal components analysis (PCA))
...
The caret package is useful in optimizing the meta-parameters that each model requires since they all require user input. The following meta-parameters will be optimized for each model:

-   Logistic Regression: both logistic regressions (PCA and noPCA) will be optimized for $\alpha$ (the elastic net mixing parameter, i.e. mix between L1 and L2 norm) and $\lambda$ (the regularization penalty size)

-   Naive Bayes: No meta parameters needed since probabilities are just based on Baye's theorem.

-   K-nearest neighbors: **K**

-   Random Forest: **mtry** (number of variables to possibly split at in each node) will be optimized while holding **split rule** and **min node size** will be held constant at "genie" and 1, respectively. Since random forests is a computationally expensive process, the number of trees to grow will be kept at 100, a decrease from the default of 500 trees.

Note that 80% of the NCDOT crash dataset will be used for training, and therefore 20% will be used for testing the model on new data. Cross validation will be used to improve performance on new data. Three times repeated 10-fold cross validation will carried out on all models except random forests since the time it takes for the model to run is excessive for a laptop. For random forest model, the 10-fold cross validation will not be repeated three times.
...
-   AmbulanceR indicates that ambulance was called.  By this point it's known to be serious, kind of like cheating if we use this variable.
-   Variables of the form NumBicsX are missing more than 50% of data
...
The amount of data missing throughout the dataset is noticeable and should imputed if possible.
...
```{r explore_bike_crashes, echo=FALSE, message=FALSE, warning=FALSE}
...
numerical_features <- c('BikeAge', 'DrvrAge', 'CrashHour', 
                        'Latitude', 'Longitude')

num_bar_plots <- bike_crashes_clean %>%
  ...
  imap(
    ~ggplot(
        ...
        geom_boxplot() +
        ggtitle(str_c(.y, ': Box Plot')) +
        ylab(.y) +
        xlab('')
  )

for(var in names(num_bar_plots)) {
  print(num_bar_plots[[var]])
}
```

<!-- ## Visualize categorical variables -->

<!-- Bar plots and top-10 summaries have been produced below for all categorical features that will be attempted to be included in the modeling process. -->
...
<!-- The following features CrashID, BikeInjuryCat will not be summarized as these features will not be used in the modeling process. -->

```{r cat_features_bar_plots, fig.height=4, fig.width=6, message=FALSE, warning=FALSE, include=FALSE}
categorical_features <- c(names(bike_crashes_clean), 'CrashYear')
no_need_4_plot <- c(
  'AmbulanceR',
  'BikeAgeGrp',
  'BikeAlcFlg',
...
cat_bar_plots <- bike_crashes_clean %>%
  ...
  imap(
    ~ggplot(
        ...
        ggtitle(str_c(.y, ': Category Proportions')) +
        xlab(.y) +
        ylab('Relative frequency') +
        theme(axis.text.x = element_text(angle = 15, hjust = 1, size = 7))
  )

for( var in names(cat_bar_plots)) {
  print(cat_bar_plots[[var]])
}
```

```{r top_10_cat_levels, message=FALSE, warning=FALSE, include=FALSE, results='asis'}
top_levels <- bike_crashes_clean %>%
  select(for_tables) %>%
  map(summary) %>%
  map(
    ~as.data.frame(.x) %>%
...
# print out kabled tables
for(var in names(top_levels)) {
  print(kable(top_levels[[var]], caption = str_c('Top 10 levels for ', var), digits = 2))
}
```
...
```{r create_dummies, echo=FALSE, message=FALSE, warning=FALSE}
...
bike_crash_dummies <- bike_crash_dummies %>% 
  ...
  select(
    # these variables are used to derive the serious bike injury response
    -matches('Disc'),
    -matches('Cat'),
  )
...
```{r corr_vars, echo=FALSE, message=FALSE, warning=FALSE}
...
tibble(
  `Correlated Vars` = correlated_vars[1:21],
  `Correlated Vars contin.` = correlated_vars[22:42]
) %>% kable(
  caption = 'Variables removed with high pairwise correlation (rho>0.75)',
)
...
```{r save_derived_data, echo=FALSE, message=FALSE, warning=FALSE}
...
# Create partitions - use same seed
set.seed(24)
...
<!-- ## Remove near-zero variance features from training dataset -->
...
```{r near_zero_variance, echo=FALSE, message=FALSE, warning=FALSE}
...
print(str_c(length(preproc_mdl$method$remove), ' dummy variables removed for near-zero variance'))
...
Three times repeated 10-fold cross-validation will carried out on all models below unless otherwise noted. In some cases, the computation method is quite complex and is very slow to compute (e.g. random forests). In these cases, repeated cross-validation will be reduced to non-repeated cross-validation.
...
```{r logistic_model, echo=TRUE, message=FALSE, warning=FALSE}
set.seed(42)
...
```{r logistic_PCA_model, echo=TRUE, message=FALSE, warning=FALSE}
set.seed(42)
...
```{r naive_bayes, echo=TRUE, message=FALSE, warning=FALSE}
set.seed(42)
...
```{r random_forests, echo=TRUE, message=FALSE, warning=FALSE}
set.seed(42)
...
```{r pred_perf_fn, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r linear_preds, echo = FALSE, message=FALSE, warning=FALSE}
...
```{r naive_bayes_pred, echo = FALSE, message=FALSE, warning=FALSE}
# Naive Bayes
...
```{r knn_pred, echo = FALSE, message=FALSE, warning=FALSE}
# K-nearest Neighbors
...
```{r rf_pred, echo = FALSE, message=FALSE, warning=FALSE}
# Random Forest
...
Logistic model, naive Bayes and random forests all perform similarly. However, both logistic models, with/without PCA included in the analysis, perform better than the others as far as optimizing sensitivity while balancing accuracy and specificity. Of the two logistic models, the one that includes the PCA with the model performs slightly better. Since the time difference for each logistic model is comparable, the PCA logistic model would seem like the most useful model. However, PCA model results are difficult to understand because they are linear combinations of other features. Therefore, the non-PCA version of the model will be used in the ensemble model since the performance isn't that different and is slightly faster. Also worth of note, if PCA is used in one model and not in others, the ensemble model is harder to interpret since it has mixed PCA features and raw feature names listed in caret::varImp.
...
```{r model_summaries, echo = FALSE, message=FALSE, warning=FALSE}
list(
  log_mdl_perf,
  log_PCA_perf,
  nb_mdl_perf,
...
```{r ensemble_classifier, echo = FALSE, message=FALSE, warning=FALSE}
set.seed(42)
# seeds to be used reproducable resampling
...
```{r ensemble_preds, echo = FALSE, message=FALSE, warning=FALSE}
...
```{r ensemble_vs_other_mdls, echo = FALSE, message=FALSE, warning=FALSE}
list(
  log_mdl_perf,
  log_PCA_perf,
  nb_mdl_perf,
...
It may seem surprising that straight roads is a good predictor serious bike crashes, but if you think about when you drive on a straight road for a while and you get into "the zone" where you don't remember the last 5-10 minutes of driving because you were on autopilot. This could be the case especially if the speeds are 40 - 55 mph where it would very difficult to avoid an obstacle in the road.
